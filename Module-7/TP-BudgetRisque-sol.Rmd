...
subtitle: |
  | Simulation d'une gestion selon un Budget Risque
  | Solution
...
daily.price.file <- "../GP/data/daily.price.rda"
...
```{r, echo=TRUE}
tmp <- read.csv("../GP/data/FEDFUNDS.csv", header=TRUE, sep=",")
...
```{r, fig.cap="Rendement cumulé", fig.pos="!H", message=FALSE,warning=FALSE}
backtestChartCumReturn(bt)
```

```{r, fig.cap="Pertes par rapport au plus haut.", fig.pos="!H", message=FALSE,warning=FALSE}
...
```{r, fig.width=6, fig.cap="Composition du portefeuille tangent avec contraintes de diversification",message=FALSE,warning=FALSE}
backtestChartStackedBar(bt, portfolio = "tangency portfolio" ,
                        legend = TRUE)
```


```{r, fig.cap="Rendement cumulé avec contraintes de diversification", message=FALSE,warning=FALSE}
backtestChartCumReturn(bt)
```
